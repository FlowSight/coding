...
- During our first phase of agent rollouts it became evident to me that the floor agent execution latency per user is 1 min. 
...
!-- //////////// -->
<!-- **Prompt:** 
    - *Describe the most impactful project you had worked on?*
...
- details:
   - It became next work item : in next release (for 3 months) I led, designed , 
   - In phase 2 for we have implemented the other part of it i.e. gen comment, thats gonna be completed now
    ...
    Each phase takes months as :
        ...
        phase 2: gen comment, LLM call mul-> compliance review
    At phase0(April-June) 
        - License purchase (estimate : NA, actual : 10k)
    At phase1(July - Sep) 
        -  License purchase (estimate : 50k, actual : 150k)
        -  Revenue (estimate : 500k, actual : 1.8mil)
    At phase2(Oct-Dec) 
        -  License purchase (estimate : 400k, actual : NA)
        -  Revenue (estimate : 5mil, actual : NA)
    At phase3 : GA(Jan - March) adoption, [future work]. 
        -  License purchase (estimate : 400k, actual : NA)
        -  Revenue (estimate : 500k, actual : 1.8mil)
...
- I mocked with 500 users concentrated in single org. Profiled key kpis like per user runtime in each tool, total RAG runtime, total flow runtime
- I observed : 5 tool execution together contributed to avg 45 secs. these tools read from same db but different tables.
- I identified agent RAG was wasting e2e service calls across 5 tools, 5 db connection setup, 3 transactions. But I had to keep the RAG in place as mandate from leadership.
-  As our workflow steps was predictable, I moved away from over relying on the RAG into creating a set of required db crud ops before RAG invocation from inside power automate flow.
- Instead of exposing 5 tools, I merged them into one tool which executed the initially evaluted setof ops by creating one db connection and in one(actually 3 as 3 entities) transaction.
- This saved : 4 round trip calls from agent to application per user + 4 db connection setups + 12 (15-3) db calls, per user.
- I further removed, replaced pessimistic locking with occ with retry. I benchmarked around 20% cases contention causes retry which results in peak latency of 10sec, which still overall yielded a much better result.
- I timed it using telemetry and sql blocking tree to find for a org of 5k user in same geo+same org with 30% concentration in same project would yield worst case 5s total wait time  transactions.
- Within a week, I had built above functional prototype single handedly that brought median latency down drastically to 8secs per user. 
- I delegated the key kpis dashboard to a junior peer.
...
-  We finished phase1 in Sep with. Agent Revenue touched 1.8mil and agent license subscription touched 150k compared the projected $500k revenue and 50K licenses. This all pivoted around the massive improvement in agent performance.
- Latest phase i.e. phase2 is to be completed in Dec 25 where we have confirmed onboarding of more premium customers.

...
```
                            ┌──────────────┐
                            │  Entra ID    │  (global auth — issues tokens)
                            │  (Azure AD)  │
                            └──────┬───────┘
                                   │ OAuth tokens
                                   ▼
┌──────────────────────────────────────────────────────────────────────────┐
│  Regional Deployment (e.g., NAM / Asia / EU)                            │
│                                                                          │
│  ┌─────────────────────┐    S2S On-Behalf-Of    ┌──────────────────┐    │
│  │  Power Automate      │◀─────────────────────▶│  BAP RP          │    │
│  │  (Scheduled Flow)    │   token for User A     │  (Impersonation  │    │
│  │                      │                        │   Token Service) │    │
│  │  ┌────────────────┐  │                        └──────────────────┘    │
│  │  │ FOR EACH user: │  │                                                │
│  │  │ 1. Get S2S     │  │                                                │
│  │  │    token        │  │                                                │
│  │  │ 2. Call MCS    │  │     Correlation ID                             │
│  │  │    with token  │  │     (self-generated:                           │
│  │  │    + corrID    │  │      ddmmyy+user+org)                          │
│  │  └────────────────┘  │                                                │
│  └──────────┬───────────┘                                                │
│             │ Native MCS connector (HTTP)                                │
│             │ Passes: user token, corrID, date range                     │
│             ▼                                                            │
│  ┌──────────────────────────────────────────────────────┐                │
│  │  MCS (Microsoft Copilot Studio) — Orchestrator only   │                │
│  │  No embedded LLM. ReAct loop, prompt assembly,        │                │
│  │  tool dispatch — all LLM calls routed via Custom API. │                │
│  │                                                       │                │
│  │  ┌────────────────────────────┐                       │                │
│  │  │ Knowledge:                 │                       │                │
│  │  │  - tool definitions        │                       │                │
...
│  │  │  - domain context          │                       │                │
│  │  └────────────────────────────┘                       │                │
│  │  Instructions: "Execute steps per Knowledge"          │                │
│  │  Guardrails: no fabricated comments, no OOB dates     │                │
│  │  GPT params: low temperature, low creativity          │                │
│  └──────┬──────────────────┬────────────────────────────┘                │
│         │                  │                                              │
│         │ Tool calls       │ ALL LLM inference                           │
│         │ (HTTP)           │ (MCS → CAPI → Azure Model)                  │
│         │                  ▼                                              │
│         │       ┌────────────────────────┐                               │
...
│  ┌────────────────────┐    ┌─────────────────────┐                       │
│  │  Action 1:          │    │  Action 2:           │                      │
│  │  Create Time Entries│    │  Update Comments     │                      │
│  │  (Custom API, C#)   │    │  (Custom API, C#)    │                      │
│  │                     │    │                      │                      │
│  │  Parallel fetches:  │    │  Parallel fetches:   │                      │
│  │  ┌───────────────┐  │    │  ┌────────────────┐  │                      │
│  │  │Task.WhenAll:  │  │    │  │Task.WhenAll:   │  │                      │
│  │  │- Assignments  │  │    │  │- Meetings      │──┼──▶ M365 Graph API    │
│  │  │- Bookings     │  │    │  │- Emails        │  │                      │
│  │  │- Existing TEs │  │    │  └────────────────┘  │                      │
│  │  └───────────────┘  │    │  Sequential:         │                      │
│  │  Then:              │    │  - Comment gen ───────┼──▶ Azure Model      │
│  │  - Compute delta    │    │    (CAPI → GPT)      │    Deployment       │
│  │  - Bulk create      │    │  - Update TEs        │                      │
...
│  │  Dataverse                                    │                        │
│  │  ┌──────────────┐  ┌────────────────────────┐│                        │
│  │  │ Time Entry   │  │ Agent Metadata Table   ││                        │
│  │  │ Assignments  │  │ (intermediate storage) ││                        │
│  │  │ Bookings     │  │ Tool A → save JSON →   ││                        │
│  │  │ Resources    │  │   return GUID to MCS → ││                        │
│  │  │              │  │ Tool B reads via GUID  ││                        │
│  │  └──────────────┘  └────────────────────────┘│                        │
│  │  OCC (RowVersion/ETag) for concurrency        │                        │
│  └───────────────────────────────────────────────┘                        │
│                                                                          │
└──────────────────────────────────────────────────────────────────────────┘
```
